...
# input
gama_old_new = 2 # 1: earlier version, 2: latest

if gama_old_new == 1:
    df_main = pd.read_csv(config.data_processed / "Dataset_processed.csv")
if gama_old_new == 2:
    df_main = pd.read_csv(config.data_processed / "Dataset_processed_GAMAlatest.csv")
#%%
df = df_main.copy()
lyrs = 9
rad_well = 2
...
gama_to_well_type = {1: 'Domestic', 2: 'DOMESTIC'} # 'Water Supply, Other', 'Municipal', 'Domestic'
well_type_select = gama_to_well_type.get(gama_old_new) 
